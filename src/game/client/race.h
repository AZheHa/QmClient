...
public:
	...
	static int TimeFromStr(const char *pStr); // x minute(s) x.xxx second(s) / x 分钟 x.xxx 秒
	static int TimeFromFinishMessage(const char *pStr, char *pNameBuf, int NameBufSize); // xxx finished in: ... / xxx 完成了地图，用时：...
