...
SEditResult<E> CEditor::DoPropertiesWithState(CUIRect *pToolBox, CProperty *pProps, int *pIds, int *pNewVal, const std::vector<ColorRGBA> &vColors)
{
	...
	for(int i = 0; pProps[i].m_pName; i++)
	{
		...
		else if(pProps[i].m_Type == PROPTYPE_ANGLE_SCROLL)
		{
			...
			auto NewValueRes = UiDoValueSelector(&pIds[i], &Shifter, "", Value, pProps[i].m_Min, pProps[i].m_Max, Shift ? 1 : 45, Shift ? 1.0f : 10.0f, Localize("Use left mouse button to drag and change the value. Hold shift to be more precise. Right click to edit as text.", "Editor"), false, false, 0);
			int NewValue = NewValueRes.m_Value;
		...
		else if(pProps[i].m_Type == PROPTYPE_ENVELOPE)
		{
			...
			auto NewValueRes = UiDoValueSelector((char *)&pIds[i], &Shifter, aBuf, CurValue, 0, m_Map.m_vpEnvelopes.size(), 1, 1.0f, Localize("Select envelope.", "Editor"), false, false, IGraphics::CORNER_NONE);
